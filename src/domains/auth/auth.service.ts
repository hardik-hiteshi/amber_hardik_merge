...
export class AuthService {
  ...
  public async signIn(
    ...
    console.log(
      'region in sign IN',
      `${request.method} ${request.url} in region ${region}`,
    );
    region = request.headers.host.split('.')[0];
    request.headers.region = region.toUpperCase();
    request['region'] = region.toUpperCase();
    const reigonArray = ['test-cmsdrive', 'cmsdrive'];
    let user;
    region = region.toLowerCase();

    if (reigonArray.includes(region)) {
      console.log('inside auth if cms region', region);
      console.log('inside auth if cms req region', request['region']);
      request['region'] = 'CMSMYCOOK';
      region = 'CMSMYCOOK';
      ...
      console.log('inside auth else normal user region', region);
