...
import { filterRules } from 'src/common/filters/user/defaultFilter';
...
export class UserService {
  public constructor(
    ...
    public userLogRepo: UserLogRepository,
  ) {}
  ...
  public async create(body: UserCreateDto, region: string): Promise<object> {
      ...
      $or: [
        {
          ...
          'contact.mail': new RegExp('^' + body.contact.mail + '$', 'i'),
        },
        {
          login: new RegExp('^' + body.login + '$', 'i'),
        },
      ],
    ...
    if (user) {
      if (user.contact.mail == body.contact.mail) {
        throw new HttpException(
          'Email address ' + body.contact.mail + ' already in use',
          HttpStatus.CONFLICT,
        );
      }
      ...
      if (user.login == body.login) {
        throw new HttpException(
          'Login ' + body.login + ' already in use',
          HttpStatus.CONFLICT,
        );
      }
    }
    ...
    if (!body.name.displayName) {
      body.name.displayName = body.login;
    }
    ...
    await this.userLogRepo.create(region, myLog);
  ...
  public async getFeaturedUser(
    ...
    return cacheFeatured && cacheFeatured[user.region]
      ? cacheFeatured[user.region].includes(user.niceName)
      : false;
  }
  ...
  public async findOne(
    ...
    response.featured = await this.getFeaturedUser(this.featuredRepo, user);
  ...
  public async findOneAndUpdate(
    ...
    console.log('niceName and region', niceName, region);

    const userData = await this.userRepo.findOne({ niceName, region });
    console.log('userdata', userData);

    if (!userData) {
      throw new HttpException(
        'User nicename ' + niceName + ' not found',
    ...
    if (body['info']['modificationDate'] == undefined) {
      body['info']['modificationDate'] = new Date();
    }

    if (body['info']['creationDate'] == undefined) {
      body['info']['creationDate'] = userData.info.creationDate || new Date();
    }

    body['info']['modificationDate'] = new Date();

    const newNiceName =
      req['user'] && req['user']['niceName'] ? req['user']['niceName'] : 'N/A';

    body['info']['modificationSource'] =
      ip + ' - ' + newNiceName + ' - ' + req.headers['user-agent'];

    console.log('body', body);

    // body.password = await bcrypt.hash(body.password, 10);
    const updatedData = await this.userRepo.findOneAndUpdate(
      { niceName, region },
      body,
    );
